...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn do_flip_coin(account_id: &T::AccountId) -> DispatchResult {
			...
			let mut coin = CoinStorage::<T>::get(account_id).ok_or(Error::<T>::CoinDoesNotExist)?;
		...
		pub fn do_toss_coin(account_id: &T::AccountId) -> DispatchResult {
			...
			// This is very a simple approach that uses blocknunber as seed source. Never use it in
			// production.
